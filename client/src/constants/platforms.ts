export type PlatformId = 'steam' | 'playstation' | 'xbox';
...
export function normalizePlatformId(raw: string): PlatformId | null {
  ...
  if (lower === 'psn' || lower.includes('playstation') || lower.includes('ps')) return 'playstation';
  if (lower.includes('xbox') || lower.includes('microsoft')) return 'xbox';
  return null;
}

export function toBackendPlatformId(id: PlatformId): string {
  if (id === 'playstation') return 'psn';
  return id;
}
